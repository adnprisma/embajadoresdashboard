...
|---|---|---|
| `[NOMBRE DE MARCA]` | `src/config/brand.ts` | **Resuelto.** `BRAND.name = "Prisma"`. Se propaga solo — `<Logo />`, el `<title>` del sitio y el footer del shell lo leen de aquí. |
| `[LOGO]` | `src/components/layout/Logo.tsx` | **Resuelto, con excepción de procedencia — alcance ampliado el 3 de septiembre de 2026.** `<Logo variant form height showName />` sirve los 6 archivos de `public/brand/` (isotipo/firma × carbón/coral/beige — matriz completa en `DESIGN_SYSTEM.md` §9). **Los 6 están generados por IA** (`grep -l trainedAlgorithmicMedia public/brand/*.png`); el dueño de marca los aprobó a sabiendas. Alcance: interfaz interna de este dashboard **y, desde el 3 de septiembre de 2026, la propuesta de cotización que se manda a un cliente** (`/pipeline/[id]/cotizaciones/[quoteId]/imprimir`, variante beige sobre el encabezado carbón) — decisión consciente tomada con la información completa enfrente: el archivo sigue siendo generado por IA y el chunk C2PA sigue intacto (`grep -l trainedAlgorithmicMedia public/brand/*.png` lo confirma sobre `logotipo_beige.png` igual que sobre los otros 5). **Sigue sin ir en registro de marca** — eso exige el vector original, que todavía no existe. Cuando exista, se reemplazan los 6 uno a uno sin tocar la API del componente.<br><br>**Hallazgo sobre metadatos, para que nadie lo tome como atajo:** `isotipo_rojo-512.png` (generado por `scripts/generate-brand-icons.mjs` al redimensionar `isotipo_rojo.png` con sharp) da `False` en la prueba de `trainedAlgorithmicMedia` — pero no es un archivo distinto ni redibujado, es la misma imagen de origen con los metadatos borrados por el reprocesamiento de sharp, que no los preserva. Borrar metadatos no cambia la procedencia de la imagen. No uses ese archivo (ni ningún redimensionado futuro que "pase" la prueba por el mismo motivo) como si hubiera dejado de ser IA — sigue bajo esta misma excepción. |
